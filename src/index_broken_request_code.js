//try doing an ajax or httprequest for each chord under playMusic, set response type to "arraybuffer", make sure timing is maintained
//you will not to use the refs, but instead just iterate over each chord object in this.state.chords and access chord.src to get file path
...
class Quiz extends React.Component {
  constructor(props) {
    ...
    this.arrayBuffer = this.audioContext.createBuffer(2, this.audioContext.sampleRate * 3, this.audioContext.sampleRate);
  ...
  playMusic(total) {
    //var arrayBuffer = this.audioContext.createBuffer(2, audioCtx.sampleRate * 3, audioCtx.sampleRate);
    ...
    if (this.count === Number(total)) {
     ...
     console.log('proving that my source actually works');
     console.log(this.state.chords[this.count].src);
     console.log(this.count);
     var sound = this.state.chords[this.count].src;
     ...
     console.log("pre onload: " + req.readyState);
     req.send();
     req.onload = function() {
       //req.onload not being called??
       console.log('testing outside of decodeAudioData');
       ...
       console.log("post send: " + req.readyState);
     };

     console.log("post onload and send: " + req.readyState);
