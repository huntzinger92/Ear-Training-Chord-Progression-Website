...
class Quiz extends React.Component {
  ...
  renderMusic() {
    ...
    if (this.state.init) {
      ...
      if (this.state.transpositions) {
        this.detuneValue = (((Math.floor(Math.random() * 6)) - 3) * 100); //anywhere from -300 to +300 cents
  ...
  playMusic(total) {
    ...
    if (this.count === Number(total)) {
     ...
     this.count++;
     setTimeout(this.playMusic, 1550 - (this.detuneValue/2), this.state.amount); //note: timeout value has to be adjusted according to detune value because detune alters playback speed of chords
   } else {
