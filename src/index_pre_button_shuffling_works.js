...
function QuizUI(props) {
  ...
  for (var i = 0; i < actualChordNames.length; i ++) { //this code will generate necessary amount of rows with answer and three (will be 1-3, dependent later) wrong answers, stored in buttonHash
    var tempButtonList = [];
    tempButtonList.push(<button className='chordButton correct'>actualChordNames[i]</button>); //put answer in row

    var answerlessAllowed = possibleChordNames.filter(function(a) { //take out correct answer from list to generate wrong answers from
      return a !== actualChordNames[i];
    });
    var random = Math.floor(Math.random() * answerlessAllowed.length); //eventually turn into a for loop where, if allowed chords are only 2, runs once, 3, twice, 4 and above, only four
    tempButtonList.push(<button className='chordButton incorrect'>answerlessAllowed[random]</button>)
    answerlessAllowed.splice(random, 1);
    random = Math.floor(Math.random() * answerlessAllowed.length);
    tempButtonList.push(<button className='chordButton incorrect'>answerlessAllowed[random]</button>)
    answerlessAllowed.splice(random, 1);
    random = Math.floor(Math.random() * answerlessAllowed.length);
    tempButtonList.push(<button className='chordButton incorrect'>answerlessAllowed[random]</button>)
    answerlessAllowed.splice(random, 1);
    buttonHash[i] = tempButtonList;
  };

  console.log('buttonHash:');
  console.log(buttonHash);
