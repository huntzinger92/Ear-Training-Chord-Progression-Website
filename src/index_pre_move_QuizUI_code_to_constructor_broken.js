//something up with setState calls in QuizUI - right now trying to access state before it is set. Use ? : notation to set it to blue in the handleButtonStyling function
//make sure the setState solution actually updates color on click
...
class QuizUI extends React.Component {
  ...
  getButtons() { //creates an array of buttons, with each item being an array of button html contained with a div, that constitutes a row
    cleanChordNameData();
    if (this.props.chords.length > 0 && !this.buttonStyling) { //only do all this stuff if we actually have chords and not buttons
      ...
      for (var i = 0; i < this.actualChordNames.length; i++) { //this code will generate necessary amount of rows with answer and 1-3 (depending on how many allowed chords) wrong answers, stored in buttonArray
        ...
        if (i === 0) {
          ...
          tempButtonList.push(<button className='chordButton correct' style={(e) => this.handleButtonStyling(e)} value={i} onClick={(e) => this.handleClick(e)}>{this.actualChordNames[i]}</button>); //put answer in row
        };
        ...
        if (i > 0) { //don't add incorrect answers on the first chord, because it's always a given that it is a one chord
          for (var j = 0; j < incorrectAmount; j++) {
            ...
            random = Math.floor(Math.random() * answerlessAllowed.length); //if 4 or more allowed chords, 3 wrong answers, if 3 allowed chords, 2, if 2, 1
            tempButtonList.push(<button className='chordButton incorrect' style={(e) => this.handleButtonStyling(e)} value={10 * i + j} onClick={(e) => this.handleClick(e)}>{answerlessAllowed[random]}</button>)
            answerlessAllowed.splice(random, 1);
          };
        };
        //reconstruct tempButtonList here with shuffle using random splicing, then push
        this.buttonArray[i] = <div key={i.toString()}>{tempButtonList}</div>;
      };
  ...
  componentDidMount() { //putting getButtons here instead of under render
    getButtons();
  };
