//modal functionality added. Something funky with naming logic under getChords (after hitting "New chords" button, numbers are off by one?)
//keys are also not unique, probably need to use index
...
class Quiz extends React.Component {
  ...
  getChords() {
    var tempChordHolder = [soundbank[(7 - this.state.modal) % 7]]; //note: you will have to add processing later to do this for real - seventh or triad, accomodate modes, etc.
    ...
    for (var i = 0; i < this.state.amount - 1; i++) {
      var rand = this.chordsAllowed[Math.floor(Math.random() * this.chordsAllowed.length)]; //choose random chord from allowedChords, generated from handleChordAllowedChange;
      //console.log('rand:');
      //console.log(rand);
